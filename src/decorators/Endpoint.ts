import type { ObjectRule, PrimitiveRule, RuleType, SchemaType, ValidationRule, ValidationSchema } from '@evojs/validator';
import type { IncomingMessage, ServerResponse } from 'http';

import type { Constructor, Middleware, ResponseHandler } from '../classes/Application';
import type { BodyOptions, BodyType } from '../utils/parsers';
...
export type ParamSchema = Record<string, RegExp>;
...
export type QueryValue<Q extends EndpointOptions['query']> = Q extends ValidationSchema ? SchemaType<Q> : Record<string, string | string[]>;
export type BodyValue<
  BT extends EndpointOptions['bodyType'],
  B extends EndpointOptions['body'],
  BP extends EndpointOptions['bodyParser'],
  BR extends EndpointOptions['bodyRule'],
> = BT extends keyof StaticBodyTypes
  ? StaticBodyTypes[BT]
  : B extends ValidationSchema
    ? BP extends (...args: any[]) => any
      ? ReturnType<BP>
      : SchemaType<B>
    : BR extends PrimitiveRule[]
      ? RuleType<BR[number]>
      : BR extends PrimitiveRule
        ? RuleType<BR>
        : any;

type RequestEndpointOptions<O extends EndpointOptions> = Request<
AuthValue<O['authHandler']>,
QueryValue<O['query']>,
BodyValue<O['bodyType'], O['body'], O['bodyParser'], O['bodyRule']>
>;
export type EndpointDecorator<O extends EndpointOptions> = <M extends (request: RequestEndpointOptions<O>) => any>(
  prototype: InstanceType<Constructor>,
  name: string,
