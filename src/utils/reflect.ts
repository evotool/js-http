...
import type { ControllerData } from '../decorators/Controller';
import type { EndpointData } from '../decorators/Endpoint';
import type { InjectData } from '../decorators/Inject';
import type { InjectableData } from '../decorators/Injectable';
...
function getParamtypes<T = any>(constructor: Constructor): T[] {
  return (Reflect.getMetadata(DESIGN_PARAMTYPES_TOKEN, constructor) || []) as T[];
}
...
export function findControllerData(constructor: Constructor): ControllerData | undefined {
  return Reflect.getMetadata(CONTROLLER_TOKEN, constructor) as ControllerData | undefined;
}
...
export function findInjectableData(constructor: Constructor): InjectableData | undefined {
  return Reflect.getMetadata(INJECTABLE_TOKEN, constructor) as InjectableData | undefined;
}
...
export function findInjectData(constructor: Constructor, index: number): InjectData | undefined {
  return Reflect.getMetadata(INJECT_TOKEN, constructor, `${index}`) as InjectData | undefined;
}
...
export function addEndpointData(prototype: Record<string, unknown>, endpointData: EndpointData): void {
